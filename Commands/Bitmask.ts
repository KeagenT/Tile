...
import type { Position, Tile, TileGrid } from "../Models/mod.ts";
...
export class UpdateTileBitmaskCommand extends Command {
  ...
  constructor(
    { coordinates, grid }: { coordinates: Position; grid: TileGrid },
  ) {
    super();
...
export class UpdateTileBitmasksCommand extends Command {
  ...
  override execute(): void {
    this.tileBitmaskCommands.push(
      new UpdateTileBitmaskCommand({
