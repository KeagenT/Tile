...
export class DrawCommand extends Command {
  ...
  private UpdateTileBitmasksCommand: UpdateTileBitmasksCommand | undefined =
    undefined;

  constructor(
    { coordinates, grid, tile }: {
  ...
  override execute(): void {
    this.SetTileCommand = new SetTileCommand({
      coordinates: this.coordinates,
      tile: this.tile,
      grid: this.grid,
    });
    this.SetTileCommand.execute();
    this.UpdateTileBitmasksCommand = new UpdateTileBitmasksCommand({
      coordinates: this.coordinates,
      grid: this.grid,
    });
  ...
  equals(command: DrawCommand): boolean {
    return this.coordinates.equals(command.coordinates) &&
      this.tile.tileType === command.tile.tileType;
  }
}
